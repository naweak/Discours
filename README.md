...
Если Вы сменили домен в `config.php` и пересобрали фронтенд, то для того, чтобы статика загружалась с нового домена, перезагрузите apache2 и memcached командой
