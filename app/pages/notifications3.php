...
<div align="center" style="margin-bottom:1em;">
  <a class="preview" onclick="link_click(25869,23);" topic_id="25869" order_in_topic="23">Ответ на пост #xyz</a>
</div>
...
foreach ($unique_parent_topics as $parent_topic)
{
  ...
  {
  	$first_notification = end($notifications_by_parent_topic[$parent_topic]);
  	$first_notification_post = Post::findFirst
	(
	[
		"post_id = :post_id:",
		"bind" => ["post_id" => $first_notification->post_id]
	]
	);
	$first_notification_order_in_topic = $first_notification_post->order_in_topic;
    echo "<div style='background:yellow;'>Replies to topic <a href='/topic/$parent_topic#$first_notification_order_in_topic' target='_blank' class='notification_link'>#$parent_topic</a></div>";
  }
  ...
  foreach (array_reverse($notifications_by_parent_topic[$parent_topic]) as $notification)
  {
    ...
    {
      echo "<div style='background:yellow;'>New topic <a href='/topic/".$notification->post_id."' target='_blank' class='notification_link'>#".$notification->post_id."</a></div>";
    }
